...
########################################
EOF
  echo "3.4.1-rc0 30401000" > "$TMP_REPO/VERSION"
}
...
@test "check: passes on consistent state" {
  run bump --mode=check
  [ "$status" -eq 0 ]
  [[ "$output" == *"current_name=3.4.1-rc0"* ]]
  [[ "$output" == *"current_code=30401000"* ]]
}

@test "check: fails on VERSION/version.properties drift (name)" {
  echo "3.4.0-rc0 30400000" > "$TMP_REPO/VERSION"
  run bump --mode=check
  [ "$status" -ne 0 ]
  [[ "$output" == *"drift"* ]]
}
...
@test "check: fails on VERSION/version.properties drift (code)" {
  echo "3.4.1-rc0 99999999" > "$TMP_REPO/VERSION"
  run bump --mode=check
  [ "$status" -ne 0 ]
  [[ "$output" == *"drift"* ]]
}
...
@test "check: fails on leading zero in version.properties" {
  ...
  echo "3.4.1-rc8 30401080" > "$TMP_REPO/VERSION"
  run bump --mode=check
  [ "$status" -ne 0 ]
  [[ "$output" == *"leading zero"* ]]
}
...
@test "plan: keep-current preserves type when type is beta" {
  ...
  echo "3.4.1-beta0 30401000" > "$TMP_REPO/VERSION"
  run bump --mode=plan --bump-kind=build --version-type=keep-current
  [ "$status" -eq 0 ]
  [[ "$output" == *"new_name=3.4.1-beta1"* ]]
}
...
@test "plan: version-type rc switches from beta back to rc0" {
  ...
  echo "3.4.1-beta3 30401030" > "$TMP_REPO/VERSION"
  run bump --mode=plan --bump-kind=build --version-type=rc
  [ "$status" -eq 0 ]
  [[ "$output" == *"new_name=3.4.1-rc4"* ]]
}
...
@test "plan: versionCode sample 3.4.1-rc0 -> 30401000" {
  run bump --mode=check
  [ "$status" -eq 0 ]
  [[ "$output" == *"current_code=30401000"* ]]
}
...
@test "plan: version-override accepts valid version" {
  run bump --mode=plan --version-override=3.5.0-rc0
  [ "$status" -eq 0 ]
  [[ "$output" == *"new_name=3.5.0-rc0"* ]]
  [[ "$output" == *"new_code=30500000"* ]]
}
...
@test "plan: version-override rejects build=100 (out of 1-2 digit range)" {
  run bump --mode=plan --version-override=3.4.2-rc100
  [ "$status" -ne 0 ]
  [[ "$output" == *"does not match"* ]]
}

@test "plan: version-override rejects bad type" {
  run bump --mode=plan --version-override=3.4.2-alpha0
  [ "$status" -ne 0 ]
}

@test "plan: version-override rejects leading zero" {
  run bump --mode=plan --version-override=3.04.0-rc0
  [ "$status" -ne 0 ]
  [[ "$output" == *"leading zero"* ]]
}

@test "plan: version-override rejects no-op identity" {
  run bump --mode=plan --version-override=3.4.1-rc0
  [ "$status" -ne 0 ]
  [[ "$output" == *"no-op"* ]]
}

@test "plan: version-override rejects monotonicity break" {
  run bump --mode=plan --version-override=3.4.0-rc0
  [ "$status" -ne 0 ]
  [[ "$output" == *"monotonicity"* ]]
}
...
@test "plan: rejects build overflow past 99" {
  ...
  echo "3.4.1-rc99 30401990" > "$TMP_REPO/VERSION"
  run bump --mode=plan --bump-kind=build --version-type=keep-current
  [ "$status" -ne 0 ]
  [[ "$output" == *"out of range"* ]]
}
...
@test "plan: rejects patch overflow past 99" {
  ...
  echo "3.4.99-rc0 30499000" > "$TMP_REPO/VERSION"
  run bump --mode=plan --bump-kind=patch --version-type=keep-current
  [ "$status" -ne 0 ]
  [[ "$output" == *"out of range"* ]]
}
...
@test "plan: expected-current matches passes" {
  run bump --mode=plan --bump-kind=build --version-type=keep-current --expected-current=3.4.1-rc0
  [ "$status" -eq 0 ]
}
...
@test "write: rewrites both files correctly (build bump)" {
  ...
  [ "$(cat "$TMP_REPO/VERSION")" = "3.4.1-rc1 30401010" ]
}
...
@test "write: patch bump zeros build in file" {
  ...
  [ "$(cat "$TMP_REPO/VERSION")" = "3.4.2-rc0 30402000" ]
}
...
@test "write: type switch persists to both files" {
  ...
  [ "$(cat "$TMP_REPO/VERSION")" = "3.4.1-beta1 30401010" ]
}
