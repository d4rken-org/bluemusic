...
class VolumeObserverTest : BaseTest() {
    ...
    @Test
    fun `observer does not relabel external return to same level after self event is consumed`() = runTest {
        val observer = VolumeObserver(
            context = context,
            appScope = backgroundScope,
            volumeTool = volumeTool,
        ...
        audioLevels[AudioStream.Id.STREAM_MUSIC] = 7
        fakeTime += 100
        observer.dispatchVolumeChanges(true) { events += it }
    ...
    @Test
    fun `observer clears stale pending self write when first observed level is different`() = runTest {
        val observer = VolumeObserver(
            context = context,
            appScope = backgroundScope,
            volumeTool = volumeTool,
