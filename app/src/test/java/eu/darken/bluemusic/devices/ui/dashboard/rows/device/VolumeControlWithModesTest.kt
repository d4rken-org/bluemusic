...
class VolumeControlWithModesTest : BaseComposeRobolectricTest() {

    private val slider = SemanticsMatcher.keyIsDefined(SemanticsProperties.ProgressBarRangeInfo)

    private fun render(volumeMode: VolumeMode?, band: VolumeBand?) {
    ...
    fun `a band with travel leaves the slider usable`() {
        render(volumeMode = VolumeMode.Normal(0.5f), band = VolumeBand(min = 0.2f, max = 0.8f))
    ...
    fun `a band whose bounds meet disables the slider`() {
        render(volumeMode = VolumeMode.Normal(0.75f), band = VolumeBand(min = 0.4f, max = 0.4f))
    ...
    fun `an unbounded stream leaves the slider usable`() {
        render(volumeMode = VolumeMode.Normal(0.5f), band = null)
