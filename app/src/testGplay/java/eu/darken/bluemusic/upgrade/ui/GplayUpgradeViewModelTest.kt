...
class GplayUpgradeViewModelTest : BaseTest() {
    ...
    fun `restore that finds a purchase emits RestoreSucceeded`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        coEvery { repo.restorePurchaseNow() } returns checked(proInfo(mockPurchase("upgrade.premium.rewrite.v3")))
        val vm = buildVm(repo)
    ...
    @Test
    fun `default entry bounces a pro user out of the screen`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        ...
        // manage=false is the acquisition entry: it auto-closes the instant the user is Pro.
        buildVm(repo, manage = false, navCtrl = navCtrl)
    ...
    @Test
    fun `manage entry keeps a pro user on the screen`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        every { repo.upgradeInfo } returns MutableStateFlow(proInfo(mockPurchase("upgrade.pro", autoRenewing = true)))
        val navCtrl = mockk<NavigationController>(relaxed = true)
        buildVm(repo, manage = true, navCtrl = navCtrl)

        advanceUntilIdle()

        verify(exactly = 0) { navCtrl.up() }
    }
    ...
    @Test
    fun `contact support navigates to the support screen`() = runTest2(context = testDispatcher) {
        val navCtrl = mockk<NavigationController>(relaxed = true)
        val vm = buildVm(mockRepo(), navCtrl = navCtrl)
