...
class GplayUpgradeScreenTest : BaseComposeRobolectricTest() {
    ...
    fun `loaded state shows trial before iap and hides loading`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_offer_divider_or)).assertCountEquals(1)
    ...
    fun `renewing subscription owner sees a locked one-time offer and management`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_why_title)).assertCountEquals(0)
        ...
        composeRule.onAllNodesWithTag(UpgradeScreenTags.GPLAY_RESTORE).assertCountEquals(1)
        composeRule.onAllNodesWithText(context.getString(R.string.upgrade_screen_status_restore_title)).assertCountEquals(1)
        composeRule.onAllNodesWithText(context.getString(R.string.settings_support_contact_label))
            .assertCountEquals(0)
    }
    ...
    @Test
    fun `legacy one-time owner sees owned status and cannot re-buy`() {
        // F1 end-to-end: a legacy upgrade.premium purchase maps (by SKU type) to IAP ownership, so
        // the screen shows the owned-status card with no buy button — a legacy owner can never be
        // pushed into re-buying.
        val legacyPurchase = mockk<Purchase>(relaxed = true).apply {
            every { products } returns listOf(OurSku.Iap.PRO_UPGRADE_LEGACY.id)
            every { isAutoRenewing } returns false
        }
        val ownership = UpgradeRepoGplay.Info(
            gracePeriod = false,
            billingData = BillingData(purchases = listOf(legacyPurchase)),
        ).toOwnership()

        composeRule.setUpgradeContent {
            UpgradeScreen(uiState = ownedState(ownership))
        }
    ...
    fun `the inconclusive dialog offers a retry that fires the callback`() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.upgrade_screen_retry_action)).performClick()
    ...
    fun `the empty-result dialog keeps the escalation path`() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.settings_support_contact_label)).performClick()
