...
class GplayUpgradeOwnershipTest : BaseTest() {
    ...
    fun `one-time purchase maps to iap ownership`() {
        val ownership = info(mockPurchase("upgrade.premium.rewrite.v3")).toOwnership()
    ...
    @Test
    fun `legacy one-time purchase still maps to iap ownership`() {
        // F1: a legacy upgrade.premium owner is a real one-time owner. Ownership matches by SKU
        // TYPE (IAP), not a specific product id, so a legacy owner is never shown acquisition
        // offers and can never be pushed into re-buying.
        val ownership = info(mockPurchase("upgrade.premium")).toOwnership()
    ...
    fun `owning both products is represented as both`() {
        val ownership = info(
            mockPurchase("upgrade.premium.rewrite.v3"),
