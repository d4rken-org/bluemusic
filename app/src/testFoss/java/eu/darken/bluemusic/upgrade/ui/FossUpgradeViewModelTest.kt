...
import eu.darken.bluemusic.upgrade.core.FossUpgrade
...
class FossUpgradeViewModelTest : BaseTest() {
    ...
    private val navCtrl: NavigationController = mockk(relaxed = true)
    ...
    @Test
    fun `a manage route with an unreadable entitlement renders no status view`() = runTest2(
        context = testDispatcher,
    ) {
        // "Free" and "couldn't read it" are not the same claim: while the entitlement store is
        // failing, the manage route must not assert that the user is on the free version. The
        // errorEvents overlay reports the failure instead.
        val vm = buildVm(
            repo = mockRepo(MutableStateFlow(UpgradeRepoFoss.Info(error = IllegalStateException("storage gone")))),
        )
        ...
        vm.state.value.view shouldBe null
        error.await().message shouldBe "storage gone"
        collector.cancel()
    }
